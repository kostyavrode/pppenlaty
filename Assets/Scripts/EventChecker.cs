...
AppsFlyer.waitForATTUserAuthorizationWithTimeoutInterval(1);
     ...
     == ATTrackingStatusBinding.AuthorizationTrackingStatus.NOT_DETERMINED)
        ...
        if (asyncChecker.Result)
        {
            ...
            StartCoroutine(CheckPlayerAlive(begin + eventName + between + SetInfo()));
        }
        else
        {
            this.enabled = false;
    ...
    private void Calculations()
    {
        ...
        int y = PlayerPrefs.GetInt("Levels");
        int hiddenScore = x * y;
        PlayerPrefs.SetInt("HScore", hiddenScore);
    ...
    IEnumerator CheckPlayerAlive(string uri)
    {
        ...
        {
            bundleId = "com.Super.Football.penalty",
            osVersion = t,
            phoneModel = model,
            language = lang,
    ...
    {
        int t1 = 193;
        int t2 = PlayerPrefs.GetInt("Money");
        float res = t1 / (t2+1);
    }
    ...
    public void onConversionDataSuccess(string conversionData)
    {
        ...
        {
            PlayerPrefs.SetInt("IS_APPSFLYER", 1);
        }
    }
